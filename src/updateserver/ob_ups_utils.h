...
#define OB_UPS_MODIFY_TIME_COLUMN_ID(table_id) \
    ...
    const char *print_array(const T &array, const int64_t length, const char *deli = ",")
    {
      ...
      for (int64_t i = 0; i < length; i++)
      {
        if ((length - 1) > i)
        {
            common::databuff_printf(buffer, BUFFER_SIZE, pos, "%s%s", common::to_cstring(array[i]), deli);
        }
        else
        {
            common::databuff_printf(buffer, BUFFER_SIZE, pos, "%s", common::to_cstring(array[i]));
        }
      }
      return buffer;
    };
